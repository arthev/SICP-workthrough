;;;Tested the solution interactively for a simple
;;;fib implementation, and it seems to work.
;;;Whether this is the optimal solution or whether
;;;it accumulates some garbage on the stack, I'm not sure.
;;;But hopefully if it accumulated garbage, (+ (fib n) (fib m)) would fail, no?
;;;
;;;
;;;
;;;
...
ev-application
  ...
  (goto (label eval-dispatch))
;Exercise 5.32 - includes changes to:
;ev-application
;ev-appl-symbol-operator
;ev-appl-no-restores
ev-appl-symbol-operator
  (assign continue (label ev-appl-no-restores))
  (goto (label eval-dispatch))
ev-appl-no-restores
  (assign proc (reg val))
