...
(define (make-meta-stack)
  '())

(define (stack-proper s)
  (cadr s))

(define (stack-name s)
  (car s))

(define (make-named-stack name)
  (list name (make-stack)))

(define (stack-by-name metastack name)
  (cond ((null? metastack) (error "Stack not found -- STACK-BY-NAME:" name metastack))
		((equal? name (stack-name (car metastack))) (car metastack))
		(else (stack-by-name (cdr metastack) name))))

(define (make-new-machine)
  (let ((pc (make-register 'pc))
        (flag (make-register 'flag))
        (stack (make-meta-stack))
        (the-instruction-sequence '()))
    (let ((the-ops
           (list (list 'initialize-stack
					   (lambda () (for-each 
									(lambda (s) ((stack-proper s) 'initialize))
									stacks)))))
          (register-table
		  ...
		  (begin
            (set! register-table
                  (cons (list name (make-register name))
                        register-table))
			(set! stack (cons (make-named-stack name) stack))))
        'register-allocated)
...
;    (lambda ()
;      (push stack (save-tag (get-contents reg) reg-name))
;      (advance-pc pc))))

(define (make-save inst machine stack pc)
  (let* ((reg-name (stack-inst-reg-name inst))
		 (reg (get-register machine reg-name))
		 (stack (stack-proper (stack-by-name stack reg-name))))
	(lambda ()
...
;    (lambda ()
;	  (let* ((p-val (pop stack))
;			 (same-reg? (equal? (get-save-tag p-val) reg-name))
;			 (r-val (get-save-val p-val)))
;		(if same-reg?
;		  (begin
;			(set-contents! reg r-val)
;			(advance-pc pc))
;		  (error "Restore" reg-name "attempted, but last value on stack was:" p-val))))))
...
;(define (save-tag reg reg-name)
;  (list reg-name reg))
;(define (get-save-tag p-val)
;  (car p-val))
;(define (get-save-val p-val)
;  (cadr p-val))
