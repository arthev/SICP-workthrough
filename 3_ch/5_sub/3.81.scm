...
(define RCL1 (RCL 1 0.2 1 0.1))
;(display-n-lines-stream (car (RCL1 10 0)) 15)
;(display-n-lines-stream (cdr (RCL1 10 0)) 15)

;3.81

;Just grabbing some rand-update from the web
...
;Note: relies on the assumption that the request stream
;has the number to reset to as the next element of the stream
(define (random-numbers request-stream)
  (define (internal initial-value request-stream)
	(cons-stream
	  initial-value
	  (let ((s (stream-car request-stream)))
		(cond ((equal? s 'generate)
			   (internal (rand-update initial-value) (stream-cdr request-stream)))
			  ((equal? s 'reset)
			   (internal (stream-car (stream-cdr request-stream))
						 (stream-cdr (stream-cdr request-stream))))
			  (else (error "Not recognized argument for random-numbers-stream: " s))))))
  (internal random-init request-stream))

(define requesties (cons-stream 'generate
				    (cons-stream 'generate
				     (cons-stream 'generate
				      (cons-stream 'reset
				       (cons-stream 3 requesties))))))

;(display-n-lines-stream (random-numbers requesties) 10)
